...
class Core extends Header {
      ...
      loop(mix, mix.x, None, { idata => {
        ...
        val (t1, t2): (DataEle, DataEle) = mix.im match {
          ...
          case im_gti: GTI_IM => mix.tw.fold[(DataEle, DataEle)]((t01, t02))(
            fb => (
              (resolveTwid(fb.n, fb.d, fb.k, resolveH(im_gti.twim, toOE(0), idata.i)) * t01),
              (resolveTwid(fb.n, fb.d, fb.k, resolveH(im_gti.twim, toOE(1), idata.i)) * t02)))
        }
  ...
  def iniGTSkeleton(n: Option[Int]): Stat = {
    if (n.isEmpty) {
      ...
      new Stat(if(static_size.isDefined) static_size.get else sph(), 1, new Stat_GT_IM(idIM, idIM), 0, None,None)
    } else ???

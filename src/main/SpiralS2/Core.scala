...
def F2(stat: Stat): MaybeSFunction = {
 ...
 val stageme: (Dyn => Data) = (dyn: Dyn) => {
   ...
   val t01 = dyn.x.apply(resolveH(mix.im.gather(), mix.v, toOE(0)))
   val t02 = dyn.x.apply(resolveH(mix.im.gather(), mix.v, toOE(1)))
   ...
   mix.y.update(resolveH(mix.im.scatter(), toOE(0),mix.v), (t1 + t2)).update(resolveH(mix.im.scatter(), toOE(1),mix.v), (t1 - t2))
 }
...
def fuseIM(r: IMHBase, s: IMHBase, lv: AInt): IMH = {
 ...
 val ns2 = r.s1 //+ (r.s0 * 0) //last part obviously 0
 ...
 /*
 def VectorMult(base: Int, strides: Vector[Int], loopvars: Vector[Int]): Int = {
 ...
 val rx = Vector.empty[Int]
 val sx = Vector.empty[Int]

 rx.headOption.getOrElse(0) * rx.headOption.getOrElse(0)
 val t = rx.tail.zipAll(sx.tail,0,0).map(p => p._1 + rx.headOption.getOrElse(0) * p._2) */
 // r = s0 0  s1
 // s = s0 s1
}
 ...
 val stageme: (Dyn => Data) = (dyn: Dyn) => {
   ...
   sum(mix.lb,mix.y, { idata => {
     val stage1mix: Mix = {
       ...
       val s1_scatter: IMH = if (inplace) { fuseIM(mix.im.scatter(),IMH(toOE(0),m,toOE(1)),mix.v) } else IMH(toOE(0),toOE(1),m)

       val nim = GT_IM(s1_gather, s1_scatter)
       val stage1_target = if (inplace) mix.y else mix.y.create(mix.n)
       mix.copy(y = stage1_target, n = m, lb = k, im = nim, v = idata.i)
     }
...
def iniGTSkeleton(n: Option[Int]): Stat = {
 if (n.isEmpty) {
   val idIM: StatIMH = new StatIMH(0,1,1)

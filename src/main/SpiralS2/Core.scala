...
class Core extends Header {
  ...
  val basecase_size: Option[Int] = Some(32)
  ...
  def loop[A](mix: Mix, ini: Data, body: iData => Data): Data = {
    val till = mix.lb
    if (!unroll(mix))    sumFoldx(till.ev.toRep(till.a), false, ini.getdata(), body)
    else {
      till.a match {
        case x: Int => {
  ...
  def DFT(stat: Stat): MaybeSFunction = {
    ...
    val stageme: (Dyn => Data) = (dyn: Dyn) => {
      val mix = Mix(stat, dyn)
      if (basecase_size.isDefined && mix.n.ev.isRep()) { //check for base case if we have a threshold defined and n is dynamic currently
        val isbasecase = mix.n.ev.less(mix.n.a, mix.n.ev.const(basecase_size.get + 1))
        mix.n.ev._if(isbasecase, {
          binsearch(mix,mix.n.ev.toRep(mix.n.a),0,basecase_size.get)
        }, {
  ...
  def iniGTSkeleton(n: Option[Int]): Stat = {
    if (n.isEmpty) {
      ...
      new Stat(if(static_size.isDefined) static_size.get else sph(), 1, new Stat_GT_IM(idIM, idIM), 0, None)
    } else ???

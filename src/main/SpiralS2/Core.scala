...
class Core extends Header {
  ...
  def resolveH(h: IMHBase, i: AInt): AInt = ???
    ...
    val stageme: (Dyn => Data) = (dyn: Dyn) => {
      ...
      val (t1, t2): (DataEle, DataEle) = mix.im match {
        ...
        case im_gti: GTI_IM => mix.tw.fold[(DataEle, DataEle)]( (t01, t02) )(
          fb => ((resolveTwid(fb.n, fb.d, fb.k, resolveH(im_gti.twim, toOE(0))) * t01),
          (resolveTwid(fb.n, fb.d, fb.k, resolveH(im_gti.twim, toOE(1))) * t02)))
      }

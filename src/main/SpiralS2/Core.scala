...
class Core extends Header {
  ...
  def F2(stat: Stat): MaybeSFunction = {
    ...
    val stageme: (Dyn => Data) = (dyn: Dyn) => {
      ...
      val t01 = dyn.x.apply(resolveH(mix.im.gather(), toOE(0), mix.v))
      val t02 = dyn.x.apply(resolveH(mix.im.gather(), toOE(1), mix.v))

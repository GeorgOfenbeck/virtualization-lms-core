...
/**
  ...
  */
class CorewGlue (variant: BreakDown.Tree, lookup: Map[List[Int], (Int,Boolean,Boolean)], testsize: Int) extends Core(variant,lookup, testsize){
  ...
  def iniGTSkeleton(n: Option[Int], precomp: Boolean): Stat = {
    if (n.isEmpty) {
      ...
      new Stat(sph2(), if (static_size.isDefined) static_size.get else sph(), 1, new Stat_GT_IM(idIM, idIM), 0, None, None, precomp, exposeInterleavedComplexVector)
    } else ???

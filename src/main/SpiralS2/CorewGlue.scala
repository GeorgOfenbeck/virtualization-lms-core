...
class CorewGlue (variant: BreakDown.Tree, lookup: BRMaps, testsize: Int,
  ...
  def iniGTSkeleton(precomp: Boolean): Stat = {
    ...
    val t = new Stat(if (static_size.isDefined) toOEL(List.empty[Int]) else sph2(),
      if (static_size.isDefined) static_size.get else sph(), 1, new Stat_GT_IM(idIM, idIM), 0, None, None, precomp, expose)
     t

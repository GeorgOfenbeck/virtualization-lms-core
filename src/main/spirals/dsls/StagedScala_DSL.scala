...
trait StagedScala_Exp  extends PureNumericOpsExp with BaseExp with InternalFunctionsExp{
  ...
  implicit def exposeRepFromStagedComplex[T](implicit tag: Manifest[T]): ExposeRep[StagedComplex] = new ExposeRep[StagedComplex](){
    val freshExps = (u: Unit) => Vector(Arg[T](tag), Arg[T](tag))
  ...
  implicit def exposeRepFromVComplex[T]( instance_size: Int)(implicit tag: Manifest[T] , exposeComplex: ExposeRep[StagedComplex]

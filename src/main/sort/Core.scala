...
class Core extends Skeleton {
  ...
  case class MaybeSFunction[A[_], B[_], C[_], AB[_]](f: Either[StagedFunction[DynHeader[A, B, C, AB], Rep[Vector[Int]]], DynHeader[A, B, C, AB] => Rep[Vector[Int]]]) {
    def apply(dyn: DynHeader[A, B, C, AB]): Rep[Vector[Int]] = f.fold(fa => fa(dyn), fb => fb(dyn))
  }

  object MaybeSFunction {
    def apply[A[_], B[_], C[_], AB[_]](f: StagedFunction[DynHeader[A, B, C, AB], Rep[Vector[Int]]]): MaybeSFunction[A, B, C, AB] = MaybeSFunction(Left(f))

    def apply[A[_], B[_], C[_], AB[_]](f: DynHeader[A, B, C, AB] => Rep[Vector[Int]]): MaybeSFunction[A, B, C, AB] = MaybeSFunction(Right(f))
  }

  def dispatcher[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    ...
    if (stat.inline) {
      ...
      val t: StagedFunction[DynHeader[A, B, C, AB], Rep[Vector[Int]]] = doGlobalLambda(stageme, Some("Dispatch" + sstring), Some("Dispatch"))(exposarg, exposeret)
      MaybeSFunction(t)
    }
  }


  def sort[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    val sstring = stat.genSig()
    val exposarg: ExposeRep[DynHeader[A, B, C, AB]] = exposeDynHeader(stat)
    val exposeret = exposeRepFromRep[Vector[Int]]
    val stageme: (DynHeader[A, B, C, AB] => Rep[Vector[Int]]) = (dyn: DynHeader[A, B, C, AB]) => {
    ...
    /*  val fret: Rep[Vector[Int]] = if (!inline){
        val r: Rep[Vector[Int]] = _if(choose_inline(size), {
          val newmix: MixSortHeader[A, B, C, AB] = mix.copy(inline = true)
          val copystat: StatHeader[A, B, C, AB] = newmix.getStatHeader()
  ...
  def selectionsort[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    val exposarg: ExposeRep[DynHeader[A, B, C, AB]] = exposeDynHeader(stat)
    val exposeret = exposeRepFromRep[Vector[Int]]
  ...
  def inserationsort[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    val exposarg: ExposeRep[DynHeader[A, B, C, AB]] = exposeDynHeader(stat)
    val exposeret = exposeRepFromRep[Vector[Int]]
    val stageme: (DynHeader[A, B, C, AB] => Rep[Vector[Int]]) = (dyn: DynHeader[A, B, C, AB]) => {
      val mix = MixSortHeader(stat, dyn)
  ...
  def mergesort[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    val exposarg: ExposeRep[DynHeader[A, B, C, AB]] = exposeDynHeader(stat)
    val exposeret = exposeRepFromRep[Vector[Int]]
    ...
    val stageme: (DynHeader[A, B, C, AB] => Rep[Vector[Int]]) = (dyn: DynHeader[A, B, C, AB]) => {
      ...
      val lessmix = MixSortHeader.apply[A, AB, C, AB](x, low_start, low_end, basesize,stat.inline,sh.eva, sh.evab, sh.evc, sh.evab, lub3, lub4, lub3, lub4)
      val (statless, dynless) = lessmix.split()
      val lessf = sort(statless)
      val xl = lessf(dynless)
      val highmix = MixSortHeader.apply[AB, B, C, AB](x, high_start, high_end, basesize,stat.inline,sh.evab, sh.evb, sh.evc, sh.evab, lub2, lub2, lub4, lub4)
  ...
  def quicksort[A[_], B[_], C[_], AB[_]](stat: StatHeader[A, B, C, AB]): MaybeSFunction[A, B, C, AB] = {
    val exposarg: ExposeRep[DynHeader[A, B, C, AB]] = exposeDynHeader(stat)
    val exposeret = exposeRepFromRep[Vector[Int]]
    ...
    val stageme: (DynHeader[A, B, C, AB] => Rep[Vector[Int]]) = (dyn: DynHeader[A, B, C, AB]) => {
      ...
      val zz: NoRep[Int] = 0
      val lessmix = MixSortHeader[NoRep, Rep, C, Rep](less, zz, lesssize, basesize,stat.inline,cNoRep, cRep, evc, cRep, NoRepRep, RepRep, NoRepRep, RepRep)
      val greatermix = MixSortHeader[NoRep, Rep, C, Rep](greater, zz, greatersize, basesize,stat.inline,cNoRep, cRep, evc, cRep, NoRepRep, RepRep, NoRepRep, RepRep)
      val (statless, dynless) = lessmix.split()
      val lessf = sort(statless)
      val xl = lessf(dynless)
  ...
  def graphvizexport() = {
    val ini: StatHeader[Rep, Rep, Rep, Rep] = StatHeader(Const(-1),Const(-1),Const(-1), false)
  ...
  def codeexport() = {
    ...
    val ini: StatHeader[Rep, Rep, Rep, Rep] = StatHeader(Const(-1),Const(-1),Const(-1), true)

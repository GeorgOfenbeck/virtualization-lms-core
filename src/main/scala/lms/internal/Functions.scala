...
trait FunctionsExp extends Functions with BaseExp with ClosureCompare with EffectExp {
  ...
  case class ExternalLambda[CA, CR](override val f: Function1[Vector[Exp[_]], Vector[Exp[_]]], override val x: Vector[TP[_]],
                                    ...
                                    global: Boolean) extends AbstractLambda(f, x, y, hot, args, returns)
  ...
  def doLambdaDef[A, R](f: Function1[A, R], internal: Boolean, hot: Boolean, global: Boolean)
    ...
    if (internal)
      ...
      ExternalLambda(vecf, tps, block, hot, args, returns, global)
  }
  ...
  override def symsFreq(e: Any): Vector[(Exp[_], Double)] = e match {
    case TP(sym, rhs, tag) => rhs match {
      ...
      case ExternalLambda(f, x, y, hot, args, returns, global) => if (hot) freqHot(sym) else freqCold(sym)

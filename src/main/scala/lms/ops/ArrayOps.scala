...
trait ArrayOps extends Base with OverloadHack {
...
trait ArrayOpsExp extends ArrayOps with EffectExp {
  ...
  case class ArrayForeach[T](a: Exp[Array[T]], x: Exp[T], block: Block) extends Def[Unit]
  ...
  case class ArrayMap[A:Manifest,B:Manifest](a: Exp[Array[A]], x: Exp[A], block: Block) extends Def[Array[B]] {

  ...
  def DFT(stat: StatGTSkeleton): (DynGTSkeleton => Single) = {
    val outer: (DynGTSkeleton => Single) = (dyn: DynGTSkeleton) => {
      ...
      myifThenElse(cond, {
        ...
        val res = sumFold(mix.loopbound.toRep(), mix.y, {
          isingle => {
            val i = isingle.i
            val acc = isingle.s
            ...
            val stage1 = {
              ...
              mix.copy(x = mix.x, y = tmp, n = m, loopbound = k, g = s1_gather, s = s1_scatter, loopvars)
            ...
            val stage2 = {
              ...
              //fuseIM(mix.g, before_fuse_gather) //gather therefore swapped

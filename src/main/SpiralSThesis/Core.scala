...
class Core(val testsize: Int, val radix_choice: Map[Int, Int], val static_size: Option[Int] = None, val interleaved: Boolean = false, val thread: Boolean = false,
  ...
  def chooseRadix(n: AInt): AInt = n.ev.fold[Int, AInt](n.a, fna => {
    R2AInt(choose_radix(fna))
  }, fnb => {
    toOE(radix_choice(fnb))
  })
    ...
    val stageme: (Dyn => Data) = (dyn: Dyn) => {
      ...
      loop(mix, mix.x, mix.y, parx, { idata => {
        val stage1mix: Mix = {
          ...
          mix.copy(x = idata.in, y = stage1_target, n = m, lb = if (inline) k else R2AInt(k.ev.toRep(k.a)), im = nim, v = idata.i, scalars = idata.scalars)
        }
        ...
        val stage2mix: Mix = {
          ...
          mix.copy(x = dataafterS1, y = idata.out, n = k, lb = if (inline) m else R2AInt(m.ev.toRep(m.a)), im = nim, v = idata.i, tw = Some(twid), scalars = idata.scalars)
        }

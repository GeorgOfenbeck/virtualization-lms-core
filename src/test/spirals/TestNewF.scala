...
class TestNewF extends FunSpec{
  ...
  describe("trying the new style functions") {
    ...
    class DSL extends NumericOpsExp with PureFunctionsExp with ReifyPure with TransformingPure{
      ...
      implicit def exposeRepFromComplex[T](implicit tag: Manifest[T]): ExposeRep[Complex[T]] = new ExposeRep[Complex[T]](){
      ...
      implicit def exposeRepFromVComplex[T](implicit tag: Manifest[T] , instance_size: Int,
      ...
      def myVfunction[X:Numeric:TypeRep:Manifest](sample: X, size: Int): Rep[_ => _] = {
